package com.greetty.recycleviewdemo.view;
...
import com.greetty.recycleviewdemo.R;
...
public class SwitchButton2  extends CompoundButton {
    ...
    private int duration = 30; //动画持续时间，如果时间设置过大，当使用ListView或RecycleView加载列表时，
                            // 复用数据，滑动列表会出现明显的打开或关闭的效果，影响体验，需根据实际使用情况调整动画时间
    ...
    public SwitchButton2(Context context, AttributeSet attrs, int defStyle) {
        ...
        if (attrs != null) {
            ...
            if (typedArray != null) {
                withTextInterval = (int) typedArray.getDimension(R.styleable.SwitchButton2_withTextInterval, withTextInterval);
                frameDrawable = typedArray.getDrawable(R.styleable.SwitchButton2_frameDrawable);
                stateDrawable = typedArray.getDrawable(R.styleable.SwitchButton2_stateDrawable);
                stateMaskDrawable = typedArray.getDrawable(R.styleable.SwitchButton2_stateMaskDrawable);
                sliderDrawable = typedArray.getDrawable(R.styleable.SwitchButton2_sliderDrawable);
                typedArray.recycle();
            }
        }
        if (frameDrawable == null || stateDrawable == null || stateMaskDrawable == null || sliderDrawable == null) {
            frameDrawable = context.getResources().getDrawable(R.mipmap.switch_frame);
            stateDrawable = context.getResources().getDrawable(R.drawable.selector_switch_state);
            stateMaskDrawable = context.getResources().getDrawable(R.mipmap.switch_state_mask);
